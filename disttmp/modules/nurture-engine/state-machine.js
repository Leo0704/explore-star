 ...
 */
export function buildTask(lead, profile, generateHookFn) {
    // 0. 前置检查
    if (lead.opt_out)
        return null;
    if (lead.status === '已流失')
    ...
    if (!transition)
        return null;
    // 3. 检查转移条件
    if (transition.condition && !transition.condition(lead))
        return null;
    // 4. 生成钩子
    let hook = '';
    if (transition.hookType && generateHookFn) {
        hook = await generateHookFn(profile, lead, transition.hookType);
    }
    else if (transition.hookType) {
        // fallback 到 lead 字段
...
// 下一动作
// ---------------------------------------------------------------------------
export function nextActionForState(status) {
    switch (status) {
...
// 状态标记
// ---------------------------------------------------------------------------
export function markStatus(lead, to, note) {
    if (lead.status === to)
        return;
