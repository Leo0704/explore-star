 ...
 */
export function findReactivatableLeads(leads, dormantDays = DEFAULT_DORMANT_DAYS) {
 ...
 * 生成再激活任务
 */
export function reactivate(lead, opts = {}) {
 ...
 * 检查 lead 是否已达最大再激活次数
 */
export function canReactivate(lead, maxAttempts = DEFAULT_MAX_ATTEMPTS) {
 ...
 * 记录再激活尝试
 */
export function recordReactivationAttempt(lead) {
    lead.reactivation_attempts = (lead.reactivation_attempts ?? 0) + 1;
    lead.last_reactivation_at = new Date().toISOString();
}
