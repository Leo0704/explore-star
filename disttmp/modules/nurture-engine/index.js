...
// 主入口：生成每日任务
// ---------------------------------------------------------------------------
export function generateDailyTasks(leads, opts) {
...
// §3.6.2 互动效果感知
// ---------------------------------------------------------------------------
function applyInteractionFeedback(lead, noRespLimit) {
...
// §3.6.3 智能放弃判定
// ---------------------------------------------------------------------------
function applyAbandonmentLogic(lead, noRespLimit, dormantDays) {
    // 显式拒绝
    if (lead.last_task_result === '被拒') {
...
// §3.6.4 再激活
// ---------------------------------------------------------------------------
export function findReactivatableLeads(leads, dormantDays = 30) {
...
export function reactivate(lead) {
    return {
...
// 内部：根据状态生成任务
// ---------------------------------------------------------------------------
function taskForState(lead, opts) {
...
function nextActionForState(status) {
    switch (status) {
...
function markStatus(lead, to, note) {
    if (lead.status === to)
        return;
...
function getPersonaValue(profile, personaId) {
