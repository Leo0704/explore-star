 ...
 * 计算关键词转化效果
 */
export function computeKeywordAttribution(events) {
    const byKeyword = {};
    for (const e of events) {
        if (!e.keyword)
            continue;
        if (!byKeyword[e.keyword])
            byKeyword[e.keyword] = { leads: new Set(), conversions: 0 };
        byKeyword[e.keyword].leads.add(e.cid);
        if (e.to_status === '已成交')
            byKeyword[e.keyword].conversions++;
    }
    ...
    const performance = Object.entries(byKeyword).map(([keyword, stats]) => {
...
function proposeWeight(current, smoothed, global) {
...
//# sourceMappingURL=keyword-attribution.js.map