 ...
 * 启动预约监听循环，持续监听新预约事件
 */
export async function watchBookings(opts) {
    ...
    while (true) {
        try {
            ...
            for (const event of events) {
                if (seenBookings.has(event.booking_id))
                    continue;
 ...
 */
export async function syncBookingsOnce(opts) {
    const errors = [];
    let synced = 0;
    try {
        const provider = getBookingProvider();
        if (!provider)
            return { synced: 0, errors: ['无可用 BookingProvider'] };
        const events = await provider.getUpcomingBookings?.() ?? [];
        for (const event of events) {
            if (!event.lead_cid)
                continue;
...
//# sourceMappingURL=booking-listener.js.map