...
// 加微后 24h 内推送物料
// ---------------------------------------------------------------------------
export async function pushMaterial(lead, opts) {
...
function buildMessage(lead, conversion) {
...
// 转化日报生成
// ---------------------------------------------------------------------------
export async function generateConversionReport(date, opts) {
    ...
    const newLeads = all.filter((l) => inRange(l.created_at, todayStart, todayEnd));
    const newWechat = all.filter((l) => inRange(l.wechat_added_at, todayStart, todayEnd));
    const newBookings = all.filter((l) => inRange(l.booked_at, todayStart, todayEnd));
    const newClosed = all.filter((l) => inRange(l.closed_at, todayStart, todayEnd));
    const revenueToday = newClosed.reduce((s, l) => s + (l.revenue ?? 0), 0);
    const weeklyRevenue = all.filter((l) => inRange(l.closed_at, weekStart, todayEnd))
    ...
    const atRisk = all.filter((l) => {
        if (!['已加微', '已私信'].includes(l.status) || !l.wechat_added_at)
            return false;
...
// 推送转化日报
// ---------------------------------------------------------------------------
export async function pushConversionReport(report) {
...
//# sourceMappingURL=material-pusher.js.map