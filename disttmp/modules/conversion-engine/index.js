...
import { reactivateLead as doReactivate } from './reactivate.js';
import { recordEvent } from '../feedback-analyzer/event-recorder.js';
/**
 * 创建转化引擎实例
 */
export function createConversionEngine(opts) {
    return {
        async onLeadAddedWechat(cid) {
            const lead = await opts.crm.getLead(cid);
            if (!lead)
                return { pushed: false, reason: 'lead 不存在' };
        ...
        async findDormantLeads() {
            return findDormantLeads({ crm: opts.crm, conversion: opts.conversion });
        },
        async reactivateLead(cid) {
            const lead = await opts.crm.getLead(cid);
            if (!lead)
                return { success: false, reason: 'lead 不存在' };
            const result = await doReactivate(lead, opts);
            return { success: result.success, reason: result.reason };
        },
        async recordTouchpoint(cid, touchpoint) {
            await recordEvent({
...
// 加微后 24h 内推送物料
// ---------------------------------------------------------------------------
export async function handleWechatAdded(lead, opts) {
...
// 转化日报
// ---------------------------------------------------------------------------
export async function generateDailyReport(date, opts) {
    ...
    const newWechat = all.filter((l) => l.wechat_added_at ? inRange(l.wechat_added_at, todayStart, todayEnd) : false);
    const newBookings = all.filter((l) => l.booked_at ? inRange(l.booked_at, todayStart, todayEnd) : false);
    const newClosed = all.filter((l) => l.closed_at ? inRange(l.closed_at, todayStart, todayEnd) : false);
    const revenueToday = newClosed.reduce((s, l) => s + (l.revenue ?? 0), 0);
    const weeklyRevenue = all.filter((l) => l.closed_at && new Date(l.closed_at).getTime() >= weekStart)
    ...
    const hot = all.filter((l) => ['已加微', '已预约', '已诊断'].includes(l.status))
        .sort((a, b) => b.intent_score - a.intent_score).slice(0, 5);
    // At risk：加微 > 5 天未互动
    const atRisk = all.filter((l) => {
        if (l.status !== '已加微' || !l.wechat_added_at)
            return false;
...
// 推送转化日报
// ---------------------------------------------------------------------------
export async function pushDailyReport(report, notifierName = 'console') {
