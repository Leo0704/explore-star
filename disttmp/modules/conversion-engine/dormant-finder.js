 ...
 */
export async function findDormantLeads(opts) {
    ...
    for (const lead of all) {
        if (!['已加微', '已私信'].includes(lead.status))
            continue;
        if (lead.status === '已成交' || lead.status === '已流失' || lead.status === '已再激活')
            continue;
        // 最后互动时间
 ...
 * 统计沉默客户汇总
 */
export async function dormantSummary(opts) {
...
//# sourceMappingURL=dormant-finder.js.map