 ...
 * 更新当前步骤状态
 */
export async function updateStep(stepIndex, status, result, error) {
    ...
    if (error)
        state.errors.push(`[${STEP_NAMES[stepIndex]}] ${error}`);
 ...
 * 标记整个流程完成
 */
export async function markComplete(completed) {
    const state = await loadState();
 ...
 * 重置状态（新的一天）
 */
export async function resetForNewDay() {
    const now = new Date().toISOString();
