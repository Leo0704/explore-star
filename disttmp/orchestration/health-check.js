 ...
 * 全面健康检查
 */
export async function checkAll(businessDir) {
    const results = [
 ...
 * §5.4.3 今日限速状态
 */
export async function checkSafetyLimits(businessDir) {
    const checks = [];
    ...
    const budget = safetyConfig.daily_budget || {};
    const limits = safetyConfig.rate_limits || {};
    ...
    const maxTasks = budget.engagement_actions || 20;
    checks.push({
        name: 'daily_tasks',
 ...
 * §5.4.4 紧急停止开关
 */
export async function checkEmergencyStop() {
    const checks = [];
