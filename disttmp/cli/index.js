...
async function cmdInit(args) {
    const name = args[0];
    if (!name) {
        console.error('用法: explore-star init <name>');
        process.exit(1);
    }
    const src = './business.example/燃点-FDE';
    const dst = `./${name}`;
    if (!existsSync(src)) {
        console.error(`源 ${src} 不存在`);
        process.exit(1);
    }
    if (existsSync(dst)) {
        console.error(`目标 ${dst} 已存在`);
        process.exit(1);
    }
...
async function cmdDoctor(_args) {
    ...
    try {
        ...
        console.log('  ❌ opencli 不可用（请 `npm install -g @jackwener/opencli`）');
        fail++;
    }
    // 3. LLM API Key
...
async function cmdNurture(args) {
...
async function cmdInsights(_args) {
...
async function cmdSearch(args) {
    const kw = args.join(' ');
    if (!kw) {
        console.error('用法: explore-star search <keyword>');
        process.exit(1);
    }
...
async function cmdBuildRagIndex(args) {
