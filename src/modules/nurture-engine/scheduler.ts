 ...
 * 冷启动时返回默认时段
 */
export function getBestInteractionTime(
  ...
  if (insights?.best_interaction_times) {
    ...
    if (match?.hours.length) {
      // 取回复率最高的小时
      const best = match.hours.reduce((a, b) =>
        (a.rate ?? 0) > (b.rate ?? 0) ? a : b
      );
      return {
        weekday: best.weekday,
        hour: best.hour,
        minute: DEFAULT_BEST_MINUTE,  // 没有 minute 字段，用默认值
      };
    }
  }
 ...
 * 获取 persona 价值分
 */
export function getPersonaValue(
  ...
  if (insights?.persona_value) {
    ...
    if (match) return match.value_score;
  }
 ...
 */
export function sortByPersonaValue(
  leads: Lead[],
  profile: BusinessProfile,
  insights: WeeklyInsights | null
): Lead[] {
