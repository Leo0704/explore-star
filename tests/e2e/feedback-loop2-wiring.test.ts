...
describe('回路 2 闭环：feedback → 下一批 LLM', () => {
  ...
  it('【关键】analyzeBatch 收到 hookStyle 后，LLM prompt 是否被注入该风格？', async () => {
    // 这个测试直接揭开"回路 2 实际效果"的面纱
    ...
    await analyzeBatch([makeComment()], {
      profile: mockProfile,
      systemPrompt: '你是分析师',
      userTplStr: '{{#each comments}}{{video_desc}}\n{{/each}}',
      llm: llmMock,
      threshold: 0.7,
      hookStyle: '数据派案例',  // 来自 feedback 归因
    });

    expect(capturedPrompts.length).toBeGreaterThan(0);
    const fullPrompt = capturedPrompts[0];
    const containsHookHint = fullPrompt.includes('数据派案例');

    // 把发现的事实写进断言（不预设"应该包含"或"不应该包含"——让代码自己说话）
    // 如果回路 2 真的引导 LLM：containsHookHint=true，测试通过
    // 如果回路 2 只归因不打标：containsHookHint=false，测试失败——我们获得真相
    if (containsHookHint) {
      console.log('✅ 回路 2 真的引导 LLM：prompt 含「数据派案例」');
    } else {
      console.warn(
        '⚠️  回路 2 只做归因（给 lead 打标），不引导 LLM：prompt 不含「数据派案例」\n' +
        '   后果：周分析得出的"最优风格"无法影响后续 LLM 生成决策。\n' +
        '   位置：src/modules/intent-analyzer/batch.ts:81-98 userPrompt 渲染未引用 ctx.hookStyle',
      );
    }

    // 当前事实断言：当前代码下 prompt 不含 hookStyle（回路 2 只归因）
    expect(containsHookHint).toBe(true);
  });
});
