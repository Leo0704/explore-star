/**
 * 事件记录器接入集成测试（Finding F3）
 *
 * 覆盖 3 个调用点：
 *   1. task-executor/executeTasks 末尾调 recordTaskExecuted
 *   2. nurture-engine/generateDailyTasks 状态变化时调 recordStatusChange
 *   3. nurture-engine/applyAbandonmentLogic 已流失时调 recordStatusChange
 *
 * 约束：不扩 LeadEvent 枚举（只用 'lead_status_changed' | 'task_executed'）
 */
...
describe('F3 事件记录器接入', () => {
  ...
  // -------------------------------------------------------------------------
  // 接入点 1: task-executor/executeTasks 末尾调 recordTaskExecuted
  // -------------------------------------------------------------------------
  it('executeTasks: 每个 task 执行后调一次 recordTaskExecuted', async () => {
    ...
    // fire-and-forget microtask: 等所有 recordTaskExecuted 落地
    await new Promise<void>(resolve => setImmediate(resolve));

    // 每个 task 执行后应调一次 recordTaskExecuted
    expect(eventRecorderMock.recordTaskExecuted).toHaveBeenCalledTimes(3);

    // 验证参数：第一个调用第一个 task 的 lead_cid
    const firstCallArgs = eventRecorderMock.recordTaskExecuted.mock.calls[0];
    expect(firstCallArgs[0]).toBe('c1');  // lead_cid
    // metadata 中含 hook_style=default, persona=self_media
    expect(firstCallArgs[1].hook_style).toBe('default');
    expect(firstCallArgs[1].persona).toBe('self_media');
  });

  // -------------------------------------------------------------------------
  // 接入点 2: nurture-engine/generateDailyTasks 状态变化时调 recordStatusChange
  // -------------------------------------------------------------------------
  it('generateDailyTasks: lead.status 变化时调 recordStatusChange（含 metadata）', async () => {
    ...
    // fire-and-forget microtask: 等 recordStatusChange 落地
    await new Promise<void>(resolve => setImmediate(resolve));

    // lead.status 已变
    expect(lead.status).toBe('已流失');
    // 应触发一次 recordStatusChange
    ...
    expect(callArgs[0]).toBe('c1');      // cid
    expect(callArgs[2]).toBe('已流失');   // toStatus
    // metadata 至少含 persona + hook_style
    expect(callArgs[3].persona).toBe('self_media');
    expect(callArgs[3].hook_style).toBeDefined();
  });

  // -------------------------------------------------------------------------
  // 接入点 3: nurture-engine/applyAbandonmentLogic 已流失时调 recordStatusChange
  // -------------------------------------------------------------------------
  it('applyAbandonmentLogic: 0 回应上限 → 标记已流失，调 recordStatusChange', async () => {
    const { generateDailyTasks } = await import('../../src/modules/nurture-engine/index.js');

    // 0 回应达到 noRespLimit（默认 3）→ 流失
    const lead = mkLead({
    ...
    expect(lead.status).toBe('已流失');
    // applyAbandonmentLogic 路径: 0 回应 → 流失 → 调 recordStatusChange
